...
import type { CardState, CreateEventInput, EventDetails, EventSummary, JoinEventInput, PrizeDraft, PublicAudit, PublicEvent } from "../model/types";
...
export const bingoApi = {
    ...
    closeWinnerPresentation: (eventId: string, roundId: string) => http<void>(`/api/events/${eventId}/rounds/${roundId}/winner-presentation/close`, { method: "POST" }),
    finishEvent: (eventId: string) => http<void>(`/api/events/${eventId}/finish`, { method: "POST" }),
    generatePrintedCards: (eventId: string, quantity: number) => http<PrintedCard[]>(`/api/events/${eventId}/cards/printed`, { method: "POST", body: JSON.stringify({ quantity }) }),
    getPrintedCards: (eventId: string) => http<PrintedCard[]>(`/api/events/${eventId}/cards/printed`),
    assignPrintedCard: (eventId: string, cardCode: string, participantId: string) => http<void>(`/api/events/${eventId}/cards/${cardCode}/assign`, { method: "POST", body: JSON.stringify({ participantId }) }),
    activatePrintedCard: (eventId: string, cardCode: string) => http<void>(`/api/events/${eventId}/cards/${cardCode}/activate`, { method: "POST" }),
    getCard: (eventId: string, cardCode: string) => http<CardState>(`/api/events/${eventId}/cards/${cardCode}/state`),
    generateNextCard: (eventId: string, cardCode: string) => http<{ publicCode: string }>(`/api/events/${eventId}/cards/${cardCode}/next`, { method: "POST" }),
    mark: (eventId: string, cardCode: string, number: number) => http<void>(`/api/events/${eventId}/cards/${cardCode}/marks`, { method: "POST", body: JSON.stringify({ number }) }),
    getPublicEvent: (code: string) => http<PublicEvent>(`/api/public/events/${code}`),
    join: (code: string, request: JoinEventInput) => http<{ publicCode: string }>(`/api/public/events/${code}/join`, { method: "POST", body: JSON.stringify(request) }),
