...
type PrintableCard = { publicCode: string; fingerprint: string; numbers: number[][]; qrCode: string };
...
export function PrintCardsPage()
{
    ...
    return <AppShell><main className="printcards"><header className="pagehead"><div><p className="eyebrow">Cartelas impressas</p><h1>Prontas para impressão</h1></div><button className="primary" onClick={() => window.print()}>Imprimir / salvar PDF</button></header><PageState loading={cards.loading} error={cards.error} /><div className="printgrid">{printableCards.map(card => <article className="printcard" key={card.publicCode}><header><strong>BINGO</strong><span>{card.publicCode}</span></header><div className="printnumbers">{card.numbers.flatMap((row, rowIndex) => row.map((number, columnIndex) => <span key={`${rowIndex}-${columnIndex}`} className={number === 0 ? "free" : ""}>{number === 0 ? "★" : number}</span>))}</div><img src={card.qrCode} alt={`QR Code da cartela ${card.publicCode}`} /><small>Fingerprint: {card.fingerprint}</small></article>)}</div></main></AppShell>;
}
