		...
		useCallback(
			() =>
				session?.accountType === "participant"
					? bingoApi.getMyCards(page, CardsPerPage)
					: Promise.resolve({
							activeCards: [],
							history: { items: [], page: 1, pageSize: CardsPerPage, totalItems: 0, totalPages: 0 }
						}),
	...
	const authenticate = async () => {
		try {
			...
			const account = isRegistering
				? await authApi.participantRegister(name, email, password)
				...
				<main className="join auth-page">
					<header className="pagehead">
						<div>
							...
							<p className="subtitle">Entre para recuperar as cartelas adquiridas em cada evento.</p>
						</div>
					</header>
					<section className="panel">
						...
						<button
							className="primary"
							disabled={isSubmitting || !email.trim() || password.length < 12 || (isRegistering && !name.trim())}
							onClick={authenticate}
						>
							{isSubmitting ? "Aguarde..." : isRegistering ? "Criar conta" : "Entrar"}
						</button>
						<button type="button" className="navlink" onClick={() => setIsRegistering((value) => !value)}>
							{isRegistering ? "Já tenho uma conta" : "Criar conta de participante"}
						</button>
			...
			<main className="join my-cards-page">
				<header className="pagehead">
					<div>
						<p className="eyebrow">Participante</p>
						<h1>Minhas cartelas</h1>
						<p className="subtitle">Consulte códigos, situação e histórico das suas cartelas.</p>
					</div>
				</header>
					...
					<>
						<section className="panel active-cards-panel" aria-labelledby="active-cards-title">
							...
							{activeCards.length === 0 ? (
								<p>Nenhuma cartela ativa no momento.</p>
							) : (
								activeCardsByEvent.map((eventCards) => (
									<section className="active-event-group" key={eventCards[0].eventId}>
										<div className="active-event-group-header">
											<h3>{eventCards[0].eventName}</h3>
											<Link className="button" to={`/cartelas/${eventCards[0].eventId}`}>
												Abrir todas as cartelas
											</Link>
										</div>
						...
						<section className="panel" aria-labelledby="card-history-title">
							...
							<div className="card-purchase-list">
								{historyCards.length === 0 ? (
									<p>Nenhuma outra cartela para exibir.</p>
								) : (
										...
										<article key={`${card.eventId}-${card.publicCode}`}>
												...
												{card.status === "Cancelled" && (
													<small>
														...
														{card.invalidationReason ??
															card.eventCancellationReason ??
															"Cartela indisponível para este evento."}
													</small>
												)}
											</div>
											{card.status === "Assigned" ? (
												<button
													...
													onClick={() => activateCard(card.eventPublicCode, card.publicCode)}
												>
													{activatingCode === card.publicCode ? "Ativando..." : "Ativar cartela"}
												</button>
											) : (
												<span className="badge">Invalidada</span>
											)}
										</article>
									))
								)}
							...
							{totalPages > 1 && (
								<nav className="pagination" aria-label="Paginação das cartelas">
									<button type="button" onClick={() => setPage(page - 1)} disabled={page === 1}>
										Página anterior
									</button>
									<span aria-live="polite">
										Página {cards.data.history.page} de {totalPages}
									</span>
