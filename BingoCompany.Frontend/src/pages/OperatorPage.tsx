...
export function OperatorPage()
{
    ...
    const performAction = async () =>
    {
        ...
        catch (error) { setError(getErrorMessage(error, "Não foi possível concluir a ação. Atualize o painel para conferir o estado atual da rodada.")); }
    };

    const revealWinner = async () =>
    {
        try { setError(""); await bingoApi.reveal(eventId, roundId); await event.reload(); }
        catch (error) { setError(getErrorMessage(error, "Não foi possível revelar o vencedor.")); }
    };
