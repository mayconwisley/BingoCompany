...
export function EventSetupPage()
{
    ...
    const saveRound = async () =>
    {
        if (editingRoundId)
        {
            const updated = await action.execute(
                async () => { await bingoApi.updateRound(eventId, editingRoundId, roundName, stages); return true; },
    ...
    const editRound = (round: NonNullable<typeof event.data>["rounds"][number]) =>
    {
        ...
        setStages(round.stages.map(stage => ({ sequence: stage.sequence, prizeName: stage.prizeName, pattern: stage.pattern, prizeImageDataUrl: stage.prizeImageDataUrl })));
    };
    ...
    return <AppShell><main>
        ...
        {event.data && <>
            ...
            <div className="two">
                <section className="panel"><p className="eyebrow">Participantes</p><h2>Inscrições</h2><p>Abra a página pública de compartilhamento para exibir o QR Code aos participantes.</p><div className="actions"><Link className="button" to={registrationSharePath} target="_blank" rel="noopener noreferrer">Abrir QR Code de inscrição</Link><Link className="button" to={registrationPath}>Visualizar inscrição</Link><button className="primary" disabled={event.data.status !== "Draft" || action.isPending} onClick={async () => { await action.execute(() => bingoApi.openRegistration(eventId), "Inscrições abertas."); await event.reload(); }}>Abrir inscrições</button></div></section>
                <section className="panel"><p className="eyebrow">Configuração da rodada</p><h2>{editingRoundId ? "Editar rodada" : hasFinishedRound ? "Preparar próximo sorteio" : "Prêmios e regras"}</h2>{hasFinishedRound && !editingRoundId && <p>As cartelas digitais e impressas ativas serão reaproveitadas automaticamente. Antes de iniciar, aguarde participantes gerarem novas cartelas ou gere, associe e ative novas cartelas impressas se necessário.</p>}<label>Nome da rodada<input aria-label="Nome da rodada" value={roundName} onChange={input => setRoundName(input.target.value)} /></label><PrizeStageEditor stages={stages} onChange={setStages} /><div className="actions"><button className="primary" disabled={event.data.status === "Finished" || action.isPending} onClick={saveRound}>{action.isPending ? "Salvando..." : editingRoundId ? "Salvar alterações" : "Salvar e ir para operação"}</button>{editingRoundId && <button type="button" disabled={action.isPending} onClick={resetRoundForm}>Cancelar edição</button>}</div></section>
            </div>
            ...
            {event.data.rounds.length > 0 && <section className="panel"><p className="eyebrow">Operação</p><h2>Rodadas criadas</h2><div className="round-list">{event.data.rounds.map(round => <article key={round.id}><div><strong>{round.name}</strong><span>{roundStatusLabel(round.status)}</span></div><div className="actions">{round.status === "Ready" && <button onClick={() => editRound(round)}>Editar rodada</button>}<button onClick={() => openRound(round.id)}>Abrir operação</button></div></article>)}</div><button className="reveal" disabled={!canFinishEvent || action.isPending} onClick={async () => { await action.execute(() => bingoApi.finishEvent(eventId), "Evento encerrado e auditoria publicada."); await event.reload(); }}>Encerrar evento e publicar auditoria</button></section>}
