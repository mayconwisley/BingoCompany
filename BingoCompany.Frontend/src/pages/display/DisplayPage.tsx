...
export function DisplayPage()
{
    ...
    return <main className="display"><ThemeToggle className="display-theme-toggle" /><p className="eyebrow">{event.data?.name}</p>{winner ? <section className="winnerpresentation"><p>🎉 GANHADOR(A) 🎉</p><h1>{winner.participantName}</h1><p className="winnerprize">{winner.prizeName}</p>{prizeImage && <img className="display-prize-image" src={prizeImage} alt={`Prêmio: ${winner.prizeName}`} />}<p className="winnerpattern">Regra vencedora: {winningPatternLabel(winner.pattern)}</p><p>Aguardando o operador continuar o sorteio.</p></section> : isWinnerSuspense ? <section className="winnerpresentation" aria-live="polite"><p>{round?.tieBreakerRequired ? "⚖️ DESEMPATE" : "🎉 TEMOS UM VENCEDOR"}</p><h1>{round?.tieBreakerRequired ? "Confira as cartelas" : "Confira a cartela!"}</h1><p className="winnerprize">{round?.currentPrize}</p>{prizeImage && <img className="display-prize-image" src={prizeImage} alt={`Prêmio: ${round?.currentPrize}`} />}{winnerCardCodes.length > 0 && <p className="winnerpattern">{winnerCardCodes.length === 1 ? `Cartela ${winnerCardCodes[0]}` : `Cartelas: ${winnerCardCodes.join(" · ")}`}</p>}<p className="winnerpattern">{round?.tieBreakerRequired ? `${winnerDetectedCount} cartelas empataram` : "O nome será revelado pelo operador."}</p><p>O sorteio está pausado para a apresentação do resultado.</p></section> : <><section className="display-draw-layout"><div className="display-prize"><p>{round?.name || "Aguardando início"} · {round?.currentPrize || "Aguardando próxima etapa"}</p>{prizeImage && <img className="display-prize-image display-prize-image--active" src={prizeImage} alt={`Prêmio em disputa: ${round?.currentPrize}`} />}</div><DrawSuspense number={round?.drawnNumbers.at(-1)} /></section><h2>{round?.drawnNumbers.length ?? 0} pedras sorteadas</h2><div className="stages">{round?.stages.map(stage => <span className={stage.isActive ? "active" : stage.isCompleted ? "completed" : ""} key={stage.prizeName}>{stage.prizeName} · {winningPatternLabel(stage.pattern)}</span>)}</div>{round?.statistics && <section className="displaystats" aria-label="Estatísticas do sorteio"><span>{round.statistics.totalCards} cartelas</span><span>{round.statistics.oneNumberAway} a 1 pedra</span><span>{round.statistics.twoNumbersAway} a 2 pedras</span><span>{round.statistics.threeNumbersAway} a 3 pedras</span></section>}</>}<div className="displayhistory">{round?.drawnNumbers.map(number => <span key={number}>{number}</span>)}</div><footer>{event.data?.cards ?? 0} cartelas · Auditoria {round?.sequenceHash?.slice(0, 16) ?? "aguardando"}</footer></main>;
}
