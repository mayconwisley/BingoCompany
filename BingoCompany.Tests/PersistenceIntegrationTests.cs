...
public sealed class PersistenceIntegrationTests
{
    ...
    public async Task Persists_event_with_printed_card_and_participant()
    {
        ...
        await using (var write = new BingoDbContext(options)) { var e = new BingoEvent("Festa"); var participant = new Participant(e.Id, "Ana"); e.AddParticipant(participant); e.AddCard(new BingoCard(e.Id, participant.Id, CardType.Digital, new int[5, 5])); write.Events.Add(e); await write.SaveChangesAsync(); }
