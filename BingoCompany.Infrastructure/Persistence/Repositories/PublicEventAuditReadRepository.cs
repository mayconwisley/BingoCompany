...
{
	public async Task<PublicEventAuditResult?> Get(string publicCode, CancellationToken cancellationToken)
	{
		...
		var entries = await db.AuditEntries.AsNoTracking().Where(item => item.EventId == bingoEvent.Id).OrderBy(item => item.OccurredAt).Select(item => new PublicAuditEntry(item.Action, item.Details, item.OccurredAt)).ToListAsync(cancellationToken);
		...
		return new PublicEventAuditResult(new PublicAuditEventInfo(bingoEvent.Name, bingoEvent.PublicCode, bingoEvent.Status, bingoEvent.CreatedAt), participants, cards, rounds.Select(round => new PublicAuditRound(round.Name, round.Sequence, round.Status, round.SequenceHash, round.Status is RoundStatus.Finished or RoundStatus.Cancelled ? round.DrawSequence : null, round.DrawnNumbers.OrderBy(item => item.Sequence).Select(item => new PublicAuditDrawnNumber(item.Number, item.Sequence, item.DrawnAt)).ToArray(), round.Stages.OrderBy(item => item.Sequence).Select(item => new PublicAuditStage(item.PrizeName, item.Pattern, item.IsCompleted)).ToArray(), winners.Where(winner => winner.RoundId == round.Id).Select(winner => new PublicAuditWinner(participantNames.GetValueOrDefault(winner.ParticipantId, "Participante indisponível"), stageNames.GetValueOrDefault(winner.StageId, "Prêmio"), winner.IsWinner, winner.TieBreakerNumber, winner.DetectedAt, winner.RevealedAt)).ToArray())).ToArray(), entries);
	}
}
