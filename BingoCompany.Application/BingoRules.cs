...
public static class WinningPatternEvaluator
{
	...
	public static int RemainingNumbers(int[,] card, IReadOnlySet<int> markedNumbers, WinningPattern pattern)
	{
		...
		return pattern switch
		{
			...
			WinningPattern.BDiagonal => Missing(Enumerable.Range(0, 5).Select(index => (index, index))),
			WinningPattern.ODiagonal => Missing(Enumerable.Range(0, 5).Select(index => (index, 4 - index))),
			WinningPattern.XPattern => Missing(Enumerable.Range(0, 5).SelectMany(index => new[] { (index, index), (index, 4 - index) }).Distinct()),
			WinningPattern.TPattern => Missing(Enumerable.Range(0, 5).Select(column => (0, column)).Concat(Enumerable.Range(1, 4).Select(row => (row, 2)))),
			WinningPattern.Frame => Missing(Enumerable.Range(0, 5).SelectMany(row => Enumerable.Range(0, 5).Where(column => row is 0 or 4 || column is 0 or 4).Select(column => (row, column)))),
			WinningPattern.Cross => Missing(Enumerable.Range(0, 5).Select(column => (2, column)).Concat(Enumerable.Range(0, 5).Where(row => row != 2).Select(row => (row, 2)))),
