...
public sealed class PublicController(BingoDbContext db) : ControllerBase
{
    ...
    public async Task<ActionResult<object>> Get(string code)
    {
        ...
        var round = e.Rounds
            .Where(x => x.Status is RoundStatus.Drawing or RoundStatus.WinnerDetected)
            .OrderByDescending(x => x.Sequence)
            .FirstOrDefault()
            ?? e.Rounds.Where(x => x.Status == RoundStatus.Ready).OrderBy(x => x.Sequence).FirstOrDefault()
            ?? e.Rounds.OrderByDescending(x => x.Sequence).FirstOrDefault();
        var presentationStage = round?.Stages.OrderByDescending(stage => stage.Sequence).FirstOrDefault(stage => stage.IsCompleted && !stage.IsWinnerPresentationClosed);
        var winner = presentationStage is null ? null : (await db.RoundWinners.Where(x => x.RoundId == round!.Id && x.StageId == presentationStage.Id && x.IsWinner).ToListAsync()).SingleOrDefault();
        var winnerName = winner is null ? null : (await db.Participants.FindAsync(winner.ParticipantId))?.Name;
        return Ok(new { e.Id, e.Name, e.PublicCode, e.Status, e.MarkingMode, participants = await db.Participants.CountAsync(x => x.EventId == e.Id), cards = await db.Cards.CountAsync(x => x.EventId == e.Id), round = round is null ? null : new { round.Id, round.Name, round.Sequence, round.Status, round.SequenceHash, currentPrize = round.Stages.SingleOrDefault(x => x.IsActive)?.PrizeName, stages = round.Stages.OrderBy(x => x.Sequence).Select(x => new { x.PrizeName, x.Pattern, x.IsActive, x.IsCompleted }), drawnNumbers = round.DrawnNumbers.OrderBy(x => x.Sequence).Select(x => x.Number), winner = winner is null ? null : new { participantName = winnerName, prizeName = presentationStage!.PrizeName, pattern = presentationStage.Pattern } } });
    }
    [HttpPost("{code}/join")]
