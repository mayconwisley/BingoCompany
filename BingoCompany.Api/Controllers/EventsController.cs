...
public sealed class EventsController(BingoDbContext db, IHubContext<BingoHub> hub) : ControllerBase
{
    ...
    public async Task<ActionResult<object>> Draw(Guid eventId, Guid roundId)
    {
        ...
        {
            round.DetectWinner();
            foreach (var card in winners) db.RoundWinners.Add(new RoundWinner(round.Id, round.ActiveStage.Id, card.Id, card.ParticipantId!.Value, drawn.Sequence));
            if (winners.Length > 1) round.StartTieBreaker();
        }
        db.AuditEntries.Add(new AuditEntry(eventId, "Pedra sorteada", $"Rodada {round.Name}: pedra {drawn.Number} na posição {drawn.Sequence}."));
        if (winners.Length > 0) db.AuditEntries.Add(new AuditEntry(eventId, "Prêmio detectado", $"{winners.Length} cartela(s) atingiram {round.ActiveStage.PrizeName}."));
        await db.SaveChangesAsync();
        var message = new { roundId, number = drawn.Number, sequence = drawn.Sequence, winnersDetected = winners.Length };
        ...
        if (winners.Length > 0)
        {
            var notice = new { roundId, count = winners.Length, tieBreakerRequired = winners.Length > 1 };
    ...
    public async Task<ActionResult<object>> Reveal(Guid eventId, Guid roundId)
    {
        ...
        var winner = candidates.OrderByDescending(x => x.TieBreakerNumber ?? 0).First(); winner.Confirm(DateTimeOffset.UtcNow); round.FinishStage();
    ...
    public async Task<IActionResult> Mark(Guid eventId, string cardCode, MarkNumberRequest request)
    {
        ...
        if (hasWon) db.AuditEntries.Add(new AuditEntry(eventId, "Prêmio detectado", "Uma cartela manual atingiu a regra ativa."));
        await db.SaveChangesAsync();
        if (hasWon)
        {
